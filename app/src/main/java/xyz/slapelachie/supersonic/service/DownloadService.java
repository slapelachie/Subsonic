...
import static xyz.slapelachie.supersonic.domain.PlayerState.COMPLETED;
import static xyz.slapelachie.supersonic.domain.PlayerState.DOWNLOADING;
import static xyz.slapelachie.supersonic.domain.PlayerState.IDLE;
import static xyz.slapelachie.supersonic.domain.PlayerState.PAUSED;
import static xyz.slapelachie.supersonic.domain.PlayerState.PAUSED_TEMP;
import static xyz.slapelachie.supersonic.domain.PlayerState.PREPARED;
import static xyz.slapelachie.supersonic.domain.PlayerState.PREPARING;
import static xyz.slapelachie.supersonic.domain.PlayerState.STARTED;
import static xyz.slapelachie.supersonic.domain.PlayerState.STOPPED;
import static xyz.slapelachie.supersonic.domain.RemoteControlState.LOCAL;

import xyz.slapelachie.supersonic.R;
import xyz.slapelachie.supersonic.activity.SubsonicActivity;
import xyz.slapelachie.supersonic.audiofx.AudioEffectsController;
import xyz.slapelachie.supersonic.audiofx.EqualizerController;
import xyz.slapelachie.supersonic.domain.Bookmark;
import xyz.slapelachie.supersonic.domain.InternetRadioStation;
...
import xyz.slapelachie.supersonic.receiver.AudioNoisyReceiver;
import xyz.slapelachie.supersonic.receiver.MediaButtonIntentReceiver;
import xyz.slapelachie.supersonic.util.ArtistRadioBuffer;
import xyz.slapelachie.supersonic.util.ImageLoader;
import xyz.slapelachie.supersonic.util.Notifications;
import xyz.slapelachie.supersonic.util.SilentBackgroundTask;
import xyz.slapelachie.supersonic.util.Constants;
import xyz.slapelachie.supersonic.util.MediaRouteManager;
import xyz.slapelachie.supersonic.util.ShufflePlayBuffer;
import xyz.slapelachie.supersonic.util.SimpleServiceBinder;
import xyz.slapelachie.supersonic.util.UpdateHelper;
import xyz.slapelachie.supersonic.util.Util;
import xyz.slapelachie.supersonic.util.compat.RemoteControlClientBase;
import xyz.slapelachie.supersonic.util.tags.BastpUtil;
...
public class DownloadService extends Service {
    ...
    private List<DownloadFile> originalList = new ArrayList<DownloadFile>();
    ...
    public synchronized void shuffle() {
        if (!isShuffled) {
            originalList = new ArrayList<>(downloadList);
            ...
            downloadList = new ArrayList<>(originalList);
            currentPlayingIndex = originalList.indexOf(currentPlaying);
...
//			resetPlaybackSpeed();
    ...
    public synchronized void previous() {
        ...
        if (getPlayerPosition() > 5000 || (index == 0 && getRepeatMode() != RepeatMode.ALL)) {
            ...
            play(index - 1, playerState != STOPPED && playerState != IDLE);
        }
    }
    ...
    public synchronized void next(boolean forceCutoff, boolean forceStart) {
        ...
        if (index != -1 && nextPlayingIndex < size()) {
            play(nextPlayingIndex, playerState != STOPPED && playerState != IDLE || forceStart);
        }
    }
